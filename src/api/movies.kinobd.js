...
const resolveProviderName = (provider, value) => {
  const fromSource = String(value?.source || value?.provider || '')
    .trim()
    .toUpperCase()
  if (fromSource) return fromSource

  return String(provider || 'player')
    .trim()
    .toUpperCase()
}
...
const buildPlayersMap = (items = []) => {
  ...
  for (const item of items) {
    ...
    const baseProvider = resolveProviderName(item?.source || item?.provider || 'kinobd', item)
    if (isHiddenProvider(baseProvider)) continue

    const baseKey = `KINOBD>${baseProvider}`
    const key = ensureUniqueKey(players, baseKey)

    players[key] = {
      name: key,
      translate: baseProvider,
...
const toProviderPlayersMap = (providerMap = {}) => {
  ...
  for (const [provider, value] of providers) {
    ...
    const baseLabel = resolveProviderName(provider, value)
    if (isHiddenProvider(baseLabel)) continue
...
const getRandomMovie = async () => {
  const { data } = await apiCall((api) =>
    api.get('/api/films/top', { params: { page: 1, per_page: 50 } })
  )
  const rows = Array.isArray(data?.data) ? data.data : []
