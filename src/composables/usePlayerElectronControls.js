...
const DESKTOP_APP_URL = 'https://t.me/Akaihoho'
const DESKTOP_ONLY_MESSAGE = 'Доступно только в приложении Akaiho Desktop'
...
export const usePlayerElectronControls = ({ isElectron, playerStore, playerIframe, kpId }) => {
      ...
      try {
        ...
        if (videos.length > 0) {
          ...
          if (currentVideoElement.value !== video) {
            ...
            compressorEnabled.value !== currentCompressorState.value &&
            mediaSource.value
          ) {
  ...
  const openAppLink = () => {
    const appUrl = `akaiho://#${kpId.value}`
    try {
