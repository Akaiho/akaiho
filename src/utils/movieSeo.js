...
const FALLBACK_DESCRIPTION =
  'Akaiho - online movie and TV streaming with collections, ratings, and simple navigation.'
const SITE_NAME = 'Akaiho'
...
export const buildMovieSeo = (movieLike = {}, kpIdOverride = null) => {
  ...
  const baseTitle = String(
    movieLike?.title || movieLike?.name_ru || fallbackEntry?.title || identity.preferredTitle || ''
  ).trim()
  ...
  const description = String(
    ...
    .trim()
  const poster = String(
    movieLike?.poster_url || movieLike?.poster || movieLike?.cover || fallbackEntry?.poster || ''
  ).trim()
