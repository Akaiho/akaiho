...
const fetchMovieInfo = async (updateHistory = true) => {
  try {
    const normalizedKpId = kp_id.value.startsWith('shiki')
      ? String(kp_id.value).replace(/\D+/g, '')
      : kp_id.value
    const response = await getKpInfo(normalizedKpId, authStore.token)
