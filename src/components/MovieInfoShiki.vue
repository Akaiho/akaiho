    ...
    <div class="content">
      ...
      <div v-if="movieInfo && !infoLoading" class="content-card">
        ...
        >
          ...
          <!-- Shikimori Rating -->
          <div v-if="movieInfo.rating_shikimori">
            <a
              :href="`https://shikimori.io/animes/${movieInfo.shikimori_id || cleanShikiId}`"
              target="_blank"
              rel="noopener noreferrer"
              class="rating-link"
              :title="`Shikimori: ${Number(movieInfo.rating_shikimori)?.toFixed(1) || 'N/A'} (${movieInfo.shikimori_votes || 0} голосов)`"
            >
              ...
              <span v-if="movieInfo.shikimori_votes" class="vote-count">
                Оценок: {{ formatRatingNumber(movieInfo.shikimori_votes) }}
              </span>
              <img
                src="/src/assets/icon-external-link.png"
                alt="Внешняя ссылка"
...
const shiki_id = ref(String(route.params.shiki_id || ''))
const cleanShikiId = computed(() => shiki_id.value.replace(/^shiki/i, ''))
const playerKpId = computed(() =>
  shiki_id.value.startsWith('shiki') ? shiki_id.value : `shiki${cleanShikiId.value}`
)
...
const fetchMovieInfo = async () => {
  try {
    ...
    // Fetch Shikimori direct (CORS ok)
    const shikiRes = await axios.get(`https://shikimori.io/api/animes/${cleanShikiId.value}`)
    const shikiData = shikiRes.data
    const calculateShikiVotes = (data) => {
      let total = 0
      if (data.scoresStats) {
        for (const score of Object.values(data.scoresStats)) {
          total += score.count || 0
        }
      }
      return total || data.rates || 0
    }

    movieInfo.value = {
      shikimori_id: cleanShikiId.value,
      name_ru: shikiData?.russian || '',
      name_en: shikiData?.name || '',
      rating_shikimori: shikiData?.score,
      shikimori_votes: calculateShikiVotes(shikiData),
      year: shikiData?.aired_on ? new Date(shikiData.aired_on).getFullYear() : '',
      type: shikiData?.status,
      description: shikiData?.description || '',
      poster_url: `https://shikimori.io${shikiData?.image?.original || ''}`,
      title: shikiData?.russian || shikiData?.name || 'Аниме',
      players: {}
    }

    navbarStore.setHeaderContent({
      text: movieInfo.value.title,
      imageUrl: movieInfo.value.poster_url
    })

    setDocumentTitle()
    backgroundStore.updateMoviePoster(movieInfo.value.poster_url)
  } catch (error) {
    errorMessage.value = 'Ошибка загрузки аниме: ' + (error.message || 'Попробуйте другой ID')
    console.error(error)
  } finally {
...
const setDocumentTitle = () => {
  if (movieInfo.value?.title) {
    document.title = `${movieInfo.value.title} - Akaiho`
  }
}
...
watch(
  ...
  async (newShikiId) => {
    if (newShikiId && newShikiId !== shiki_id.value) {
      ...
      shiki_id.value = String(newShikiId)
