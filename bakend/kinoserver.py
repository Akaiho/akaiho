...
class Settings:
    ...
    kinobox_base_url: str = os.getenv("KINOBOX_BASE_URL", "https://api.kinobox.tv")
    kinobox_referer: str = os.getenv("KINOBOX_REFERER", "https://tapeop.dev/")
    kinobox_origin: str = os.getenv("KINOBOX_ORIGIN", "https://tapeop.dev")
        ...
        for origin in os.getenv(
            ...
            ",".join(
                [
                    ...
                    "https://akaiho.github.io/akaiho",
                    "https://akaiho.vercel.app",
                    "https://akaiho.onrender.com",
                    "http://localhost:5173",
                    "http://127.0.0.1:5173",
                ]
            ),
...
def register_routes(app: Sanic) -> None:
    ...
    @app.get("/shiki_rating/<kp_id:str>")
    async def shiki_rating(_: Request, kp_id: str):
        """Shikimori rating by Kinopoisk ID (external IDs lookup)"""
        kp_id = clean_digits(kp_id)
        if not kp_id:
            return json({}, status=404)
        cache_key = f"shiki_rating:{kp_id}"

        async def factory():
            # 1. Kinopoisk external IDs
            ensure_provider_token(app.ctx.settings.kinopoisk_token, "Kinopoisk token missing")
            external_payload = await fetch_json(
                app,
                f"{KINOPOISK_API_BASE}/api/v2.2/films/{kp_id}/external",
                headers={"X-API-KEY": app.ctx.settings.kinopoisk_token}
            )
            shiki_candidates = external_payload.get("externalIds", {}).get("shikimoriAnime", [])
            if not shiki_candidates:
                return {}
            shiki_id = clean_digits(shiki_candidates[0])  # First Shiki ID
            if not shiki_id:
                return {}
            # 2. Shikimori data
            return await shikimori_info(app, shiki_id)

        result = await app.ctx.cache.get_or_set(cache_key, MOVIE_CACHE_TTL_SECONDS, factory)
        return json({
            "shiki_id": result.get("shikimori_id"),
            "rating_shikimori": result.get("rating_shikimori"),
            "shikimori_votes": result.get("shikimori_votes")
        })
    ...
    async def shiki_to_kp(_: Request, shiki_id: str):
        ...
        if not kp_id:
            return json({}, status=404)
        return json({"id_kp": kp_id, "kinopoisk_id": kp_id})
    ...
    @app.get("/kinobox/players/<kp_id:str>")
    async def kinobox_players(request: Request, kp_id: str):
...
async def shikimori_info(app: Sanic, shiki_id: str) -> dict[str, Any]:
    ...
    async def factory() -> dict[str, Any]:
        ...
        kodik_payload = await fetch_json(
            app,
            f"{KODIK_API_BASE}/search",
            params={"token": app.ctx.settings.kodik_token, "shikimori_id": shiki_id},
        )
        first_kodik = (kodik_payload.get("results") or [{}])[0]

        # Fetch Shikimori data
        shikimori_payload = await fetch_json(
            app,
            f"https://shikimori.io/api/animes/{shiki_id}",
            headers={"Accept": "application/json"},
        )

        return {
            "shikimori_id": shiki_id,
            "kinopoisk_id": first_kodik.get("kinopoisk_id"),
            "name_ru": first_kodik.get("title") or shikimori_payload.get("russian") or "",
            "name_en": first_kodik.get("title_orig") or shikimori_payload.get("english") or "",
            "name_original": first_kodik.get("title_orig") or shikimori_payload.get("name") or "",
            "slogan": first_kodik.get("other_title") or shikimori_payload.get("slogan") or "",
            "year": str(first_kodik.get("year") or shikimori_payload.get("aired_on", {}).get("year") or ""),
            "poster_url": first_kodik.get("poster") or shikimori_payload.get("image", {}).get("original") or "",
            "poster_url_preview": first_kodik.get("poster") or shikimori_payload.get("image", {}).get("preview") or "",
            "rating_shikimori": parse_float(shikimori_payload.get("score")),
            "shikimori_votes": parse_float(shikimori_payload.get("rates", 0)),
            "screenshots": [],
            "videos": [],
            "staff": [],
            "sequels_and_prequels": [],
...
async def get_cdnmovies_players(
    ...
    if "iframe" not in response_text.lower():
        return {}
    iframe = f"https://ugly-turkey.cdnmovies-stream.online/kinopoisk/{kp_id}/iframe?domain=akaiho.github.io"
...
async def kinobox_players_by_kp(app: Sanic, kp_id: str, *, title: str = "") -> list[dict[str, Any]]:
    kp_id = clean_digits(kp_id)
    if not kp_id:
        return []
    cache_key = f"kinobox_players:{kp_id}:{title.lower()}"

    async def factory() -> list[dict[str, Any]]:
        settings: Settings = app.ctx.settings
        base_url = str(settings.kinobox_base_url or "https://api.kinobox.tv").rstrip("/")
        headers = {
            "Accept": "application/json",
            "Referer": settings.kinobox_referer,
            "Origin": settings.kinobox_origin,
        }
        params: dict[str, Any] = {"kinopoisk": kp_id}
        if title:
            params["title"] = title

        payload = await fetch_json(app, f"{base_url}/api/players", headers=headers, params=params)
